...
if (isset($_POST['submit'])) {
    ...
    if(!isset($_POST['gtoken'])){
        ...
        $sql     = "SELECT * FROM tbl_student WHERE exam = $exam_id AND token = 0";
        ...
        if ($count) {  
            ...
            // SELECT @token:=99; UPDATE tbl_student SET token = @token:=@token+1 WHERE exam = $exam_id
            // $token   = mt_rand(100, 999);

            // $query   = "SELECT @token:=100 UPDATE tbl_student SET token = @token:=@token+1 WHERE exam = $exam_id";

            // $query   = "UPDATE tbl_student (SELECT @token:=100) SET token = @token:=@token+1 WHERE exam = $exam_id";

            $sql   = "SELECT @token:=100";
            $res   = mysqli_query($conn, $sql);

            if($res){

                $query   = "UPDATE tbl_student SET token = @token:=@token+1 WHERE exam = $exam_id";
